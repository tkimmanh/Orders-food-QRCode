...
import { useIndicatorQuery } from "@/queries/useIndicator";
...
export default function DashboardMain() {
  ...
  const revenue = data?.payload.data.revenue ?? 0; // tổng doanh thu
  const customer = data?.payload.data.guestCount ?? 0; // tổng số khách
  const order = data?.payload.data.orderCount ?? 0; // tổng số đơn hàng
  const servingTable = data?.payload.data.servingTableCount ?? 0; // số bàn đang phục vụ
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{servingTable}</div>
          </CardContent>
        </Card>
      </div>
